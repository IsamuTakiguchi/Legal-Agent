...
- 記録は `logs\install-*.log`（導入時）、`logs\start-*.log`（起動のたび）、`logs\server-*.log`（サーバーの出力）に残ります。古いものは自動で削除されます。うまく動かないときは、メモ帳で開いた `status.txt` やログの内容を送っていただければ原因が分かります。
...
- **自動更新**: `start.bat`（デスクトップの Legal-Agent）を押すたびに GitHub の最新版を確認し、差分があれば入れ替えます（`.env`・索引・履歴・ブラウザプロファイルは保持）。サーバーが既に動いていても、更新が入ったときは自動で再起動して新しい版に切り替えます。稼働中も 1 時間ごとに確認し、新しい版があれば画面上部に「次回起動時に自動更新」と表示するので、そのときは `start.bat` をもう一度押してください。止めたい場合は `.env` に `LEGAL_AGENT_AUTO_UPDATE=false`。ZIP で展開した環境では初回起動時に一度だけ同じ版を取り直します（バージョンの記録のため）。
...
- Windows の「ファイル オンデマンド」でクラウドのみになっている PDF は、**許可制**です。アプリが勝手にダウンロードすることはなく、左の「書籍索引」に「ダウンロード許可待ち N 冊」と出るので、「許可する…」を押して一覧から選ぶと、その PDF だけ OneDrive から取得して索引します（許可は `data\download_approvals.json` に記憶）。「以後すべて自動で許可」を選ぶか、`.env` に `LEGAL_AGENT_AUTO_DOWNLOAD_CLOUD_PDFS=true` を書くと従来どおり自動ダウンロードになります。ターミナルなら `python -m legal_agent index --allow-all`。
- 既に PC 上にある（同期済みの）PDF は、許可なしでそのまま索引します。
