...
| 文献 | LEGAL LIBRARY（legal-library.jp） | 規約により既定ではアクセスせず、手動検索用リンクを案内（下記「利用規約の確認結果」参照） |
...
両サービスの利用規約原文を確認しました。詳細と条文の引用は [docs/TERMS_REVIEW.md](docs/TERMS_REVIEW.md) を参照してください。

| サービス | 結論 | アプリの既定動作 |
|---|---|---|
| LEGAL LIBRARY | 第 8 条で「自動化された手段によるアクセス」「AI 等の使用」を明文で禁止。**許容されない可能性が高い** | アプリからはアクセスしない。利用者が自分で検索するためのリンクを案内するだけ（`LEGAL_AGENT_LEGAL_LIBRARY_ENABLED=false`） |
| TKC ローライブラリー | 自動化手段・AI 利用の明文禁止はなし。「明らかに異常な分量・頻度」のダウンロードと目的外利用が禁止 | 有効。1 質問あたり検索 3 回・本文 6 件・1 秒以上の間隔に制限し、本文はディスクに保存しない |

- TKC については、判例本文を Claude API に送る点（9-1 目的外利用の「複製」との関係）と、LEX/DB 等の個別規約が未確認です。docs/TERMS_REVIEW.md に TKC への照会文の例を載せています。
- LEGAL LIBRARY を使いたい場合は、運営会社（株式会社 Legal Technology）に許諾を求めてください（照会文の例も同文書にあります）。許諾が得られたら `.env` に `LEGAL_AGENT_LEGAL_LIBRARY_ENABLED=true` を追加します。
- 本アプリは **利用者本人の操作の代行** に限定し、一括クロールや本文の恒久保存は行いません。
